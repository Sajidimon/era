            ...
            <div class="py-8 px-4 mx-auto max-w-2xl lg:py-10">
                ...
                <form @submit.prevent="update">
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        ...
                        <img
                            class="w-36 h-36"
                            v-if="imagePreview || props.funnel.image"
                            :src="imagePreview || `/storage/${props.funnel.image}`"
                        />
                    ...
                    >
                        <li
                            v-for="product in funnel.products.slice(0, 8)"
                            :key="product.id"
...
const selectedProducts = ref(props.funnel.products.map((product) => product.id));
...
const form = useForm({
    ...
    subheading: props.funnel.subheading,
    image: null,
...
function update() {
    form.post(`/update-funnel/${props.funnel.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            toast("Funnel Updated Successfully", { type: "success" });
